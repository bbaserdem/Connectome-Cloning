function [ success, H ] = simulation_flicker( C )
% Simulation with quadratic hamiltonian (G=2) and synaptic flickering
% Inputs
%   C: input connectome
% Outputs
...
active_syn = 0.3;   % Active synapse density
jitter_fre = 0100;  % Jitter frequency
...
% B: Barcode matrix, (barcode,cell) and populate it from S
% Occupied synapses matrix, populated 
M = zeros(n,N);
O = false(N);
for b = 1:B
    O(S(b,1), S(b,2)) = true;
...
% List of synapses that are active
A = cell(N,2);
...
for t = 1:T
    ...
    % Jittering active synapses
    if mod(t, jitter_fre) == 1
        J = O | ( rand(N) < active_syn );   % Jittering synapses
        F = [sum(J,2), sum(J,1)'];   % Number of active synapses out/inc
        for a = 1:N
            A{a,1} = find( J(a,:) ); % List of active outgoing synapses
            A{a,2} = find( J(:,a) ); % List of active incoming synapses
        end
    end
    ...
    if r1 < R(1)
        ...
        c  = S(b,3-p);          % Origin cell
        c1 = S(b,p);            % Changing cell is S(b,p)
        if F(c,3-p) <= 1        % If only one active synapse, can't do jump
            continue
        end
        c2 = c1;                % Changed cell is a random cell not c1
        while c2 == c1
            c2 = A{c,3-p}( ceil( F(c,3-p) * rand ) );
        end
