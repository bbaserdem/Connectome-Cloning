function [ O, C, conid, VID, stepSize ] = cc_video( N, D, E, H )
%CONNCLONE Simulation of a connectome cloning run (added time recording)
...
recordAmount = 100;
% Aim to record ~1000 steps (for 1000x1000, should result in 1GB image)
stepSize = ceil( cEstimate / recordAmount );
% Account for overhead
VID = false( N, N, 2*recordAmount );
...
for t = 1:T
    ...
    if L == 0
        ...
        step = step+1;
        VID(:,:, step) = cv_connectomeFromS(S,N);
        VID = VID(:,:,1:step);
